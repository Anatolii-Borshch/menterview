...
export default function HistoryDetailPage() {
  ...
  const scoreColor =
    session.score >= 80 ? 'text-green-600' : session.score >= 60 ? 'text-yellow-600' : 'text-red-500';
        ...
        <div className="bg-white border border-periwinkle rounded-2xl p-8 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            {[
              ...
              { label: 'Unanswered', value: session.unansweredCount },
              { label: 'Duration', value: formatTime(session.totalTime) },
              { label: 'Correctness', value: `${session.averageCorrectness}%` },
            ].map((s) => (
            ...
            >
              ...
              >
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-navy line-clamp-2">{ans.question}</p>
                  <div className="flex items-center gap-3 mt-1.5">
                    <span className="text-xs text-navy/40">
                      {formatDuration(ans.answeringTime)}
                    </span>
                    <span className={`text-xs font-medium ${
                      ans.correctness >= 80 ? 'text-green-600'
                      : ans.correctness >= 50 ? 'text-yellow-600'
                      : 'text-red-500'
                    }`}>
                      {ans.correctness}% correct
                    </span>
                  </div>
                </div>
                <svg
                  className={`w-4 h-4 text-navy/30 flex-shrink-0 transition-transform mt-0.5 ${expandedIdx === idx ? 'rotate-180' : ''}`}
              ...
              {expandedIdx === idx && (
                <div className="border-t border-periwinkle p-4 space-y-4">
                  <div>
                    ...
                    <p className="text-sm text-navy">{ans.answer || <span className="italic text-navy/30">No answer provided</span>}</p>
                  </div>
                  {ans.aiReply && (
                    <div>
                  ...
                  <div className="flex gap-4">
                    <div className="text-center">
                      <p className="text-sm font-semibold text-navy">{ans.correctness}%</p>
                      <p className="text-xs text-navy/40">Correctness</p>
                    </div>
                    <div className="text-center">
                      <p className="text-sm font-semibold text-navy">{ans.completeness}%</p>
